...
const ListingDetailsScreen = ({ route, navigation }) => {
  ...
  const handleAddToCart = async () => {
    try {
      ...
      Alert.alert('Success', 'Added to your gear cart! 🎸');
    } catch (error) {
  ...
  const renderPhoto = ({ item }) => (
    <Image source={{ uri: getImageUrl(item) }} style={styles.carouselImage} />
  );
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.carouselContainer}>
          <FlatList
            data={listing.photos && listing.photos.length > 0 ? listing.photos : ['/uploads/default-instrument.jpg']}
...
const styles = StyleSheet.create({
  ...
  footerActions: { position: 'absolute', bottom: 0, width: '100%', backgroundColor: COLORS.white, padding: 20, flexDirection: 'row', borderTopWidth: 1, borderTopColor: COLORS.border, paddingBottom: Platform.OS === 'ios' ? 40 : 20 },
  cartBtn: { width: 55, height: 55, backgroundColor: COLORS.background, borderRadius: 15, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  msgBtn: { flex: 1, height: 55, backgroundColor: COLORS.primary, borderRadius: 15, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  msgBtnText: { color: COLORS.white, fontWeight: 'bold', fontSize: 16 },
  buyBtn: { flex: 1.5, height: 55, backgroundColor: COLORS.secondary, borderRadius: 15, justifyContent: 'center', alignItems: 'center' },
  buyBtnText: { color: COLORS.primary, fontWeight: 'bold', fontSize: 16 },
  ownerFooter: { position: 'absolute', bottom: 0, width: '100%', backgroundColor: COLORS.white, padding: 20, borderTopWidth: 1, borderTopColor: COLORS.border, paddingBottom: Platform.OS === 'ios' ? 40 : 20 },
